package com.francobm.fconomy.packets.title;
...
public class TitlePacket {
    ...
    private void sendActionBarPacket(Player player, String message){
        if(!lastVersion()) {
            try {
                Object actionBarChat = getNMSClass("IChatBaseComponent").getDeclaredClasses()[0].getMethod("a", String.class).invoke(null, "{\"text\":\"" + UtilsFC.ChatColor(message) + "\"}");
            ...
            player.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(UtilsFC.ChatColor(message)));
        }
    }
    ...
    private void sendTitlePacket(Player player, String title, String subTitle, int fadeIn, int stay, int fadeOut) {
        if(!lastVersion()) {
            try {
                ...
                Object chatTitle = getNMSClass("IChatBaseComponent").getDeclaredClasses()[0].getMethod("a", String.class).invoke(null, "{\"text\":\"" + UtilsFC.ChatColor(title) + "\"}");
                ...
                Object chatSubTitle = getNMSClass("IChatBaseComponent").getDeclaredClasses()[0].getMethod("a", String.class).invoke(null, "{\"text\":\"" + UtilsFC.ChatColor(subTitle) + "\"}");
